...
- (4) aligning incentives with actual data utility through a balanced scoring system based on trading volume (30%), unique contributors (20%), and data access fees (50%). Until $1B TDVL is reached, only data access fees (50% of rewards pool) are distributed to focus on real utility. 
...
3) For this example, assume the DLP incentives pool is 300,000 VANA for the epoch, and 30 DLPs are eligible. The split of rewards is determined at the end of the epoch based on the following three performance metrics: DLP token trading volume, unique data contributors, and data access, measured as averages across the epoch. 
   ...
   **Until $1B TDVL is reached, only the data access fees portion (50%) of the rewards pool is distributed. The remaining 50% allocated to DLP token trading volume and unique data contributors is not distributed to any DLP during this phase.**
   ...
   - Scores for each category are normalized across DLPs. **Until $1B TDVL is reached, only 50% of the rewards pool is distributed based on: DLP Score = 1.00 * (DLP data access fees / total data access fees). After $1B TDVL is reached, the full rewards pool is distributed based on:** DLP Score = 0.30 * (DLP token trading volume / total DLP token trading volume) + 0.20 * (DLP unique data contributors / total unique data contributors) + 0.50 * (DLP data access fees / total data access fees). 
   - DLPs should expect performance metrics and weightings to change regularly across epochs to respond to ecosystem priorities. Data access will continue as a core component, with additional metrics weighted as needed to encourage the most important data contributions for the protocol.
...
- DLP incentives are based on: DLP token trading volume (30%), data contributors (20%), and data access fees (50%). Until $1B TDVL is reached, only the data access fees portion is distributed.
